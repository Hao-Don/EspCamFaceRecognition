...
face_id_node *match = st_face_list.head;
...
//    if(*matchname == 't'){
      if((match->id_name)==(st_face_list.head->id_name)){
      myservo1.write(90);
      Serial.println("Door Unlocked for th");
...
//    if(*matchname == 'z' ){
      if(((match->id_name)==(st_face_list.tail->id_name))&&((st_face_list.head->id_name)!=(st_face_list.tail->id_name))){
      myservo.write(90);
      Serial.println("Door Unlocked for z");
//      delay(2000);
//      myservo.write(0);
//      client.send("door_open");
//      door_opened_millis = millis(); // time relay closed and door opened
...
void loop() {
  myservo.attach(servoPin, 1000, 2000); // attaches the servo on pin  to the servo object
        ...
        if (align_face(out_res.net_boxes, image_matrix, aligned_face) == ESP_OK)
        {
          ...
          if (g_state == START_RECOGNITION  && (st_face_list.count > 0))
          {
            ...
            if (f)
            {
              ...
              matchname=f->id_name;
//              Serial.println(matchname);
//              Serial.println(*matchname);
              open_door(client);
              client.send(recognised_message);
//              Serial.println(st_face_list.count);
//              Serial.println()
//              Serial.println(match->id_name);
              int flag1=0;
//              Serial.println(st_face_list.tail->id_name);
            }
            else
            {
      ...
      else
      {
        if (g_state != START_DETECT) {
          client.send("NO FACE DETECTED");
           if(digitalRead(buttonPin) == HIGH &&( (myservo.read() >= 45) || (myservo1.read() >= 45))){
//               if((myservo.read() >= 45)){ 
                    myservo.write(0);
//                }
//                if((myservo1.read() >= 45)){ 
...
//                }
              ...
              if(digitalRead(buttonPin) == HIGH &&( (myservo.read() >= 45) || (myservo1.read() >= 45))&&(millis() - last_detected_millis > 60000)){
//               if((myservo.read() >= 45)){ 
                    myservo.write(0);
//                }
//                if((myservo1.read() >= 45)){ 
...
//           if(digitalRead(buttonPin1) == HIGH && myservo1.read() >= 45){
//                myservo1.write(0);
//              } 
            if ((myservo.read() >= 45) || (myservo1.read() >= 45) && (millis() - last_detected_millis > 1000)) {//超过10秒未关门则提示
//                Serial.println("未关门");
                client.send("未关门");
              }             
        }
